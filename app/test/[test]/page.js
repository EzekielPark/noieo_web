...
export default async function TestGroupPage({ params, searchParams }) {
  ...
  const state = await fetchBoardPage(firstPageInGroup, searchParams?.category, searchParams?.subcategory, searchParams?.lang);
  return <BoardPage {...state} lang={searchParams?.lang} />;
}
